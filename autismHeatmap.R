set.seed(12345)
ha = HeatmapAnnotation(df = data.frame(type = c(rep("Control", 21), rep("Autism", 23))), col = list(type = c("Autism" = "red4", "Control"="blue4")))
#column_tree = hclust(dist(t(meth)), method = "ward.D2")
ht_list = Heatmap(meth1, clustering_method_columns = "ward.D2", name = "Methylation", col = colorRamp2(c(0, 0.25, 0.5, 0.75, 1), c("green4","aquamarine4", "violet", "firebrick3","red4")),
...
pdf("Autism-Methylation2.pdf", width = 10, height = 10)
