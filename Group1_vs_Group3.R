...
## Write results
write.csv(resdata, file="Group1 Vs Group3 DEG.csv")
resdata_0.05 <- resdata[(abs(resdata$log2FoldChange)> 1 & resdata$padj <= 0.05) ,]
write.csv(resdata_0.05, file="Group1 Vs Group3 DEG log2FC 1.0 FDR 0.05.csv")
resdata_0.01 <- resdata[(abs(resdata$log2FoldChange)> 1.5 & resdata$padj <= 0.01) ,]
write.csv(resdata_0.01, file="Group1 Vs Group3 DEG log2FC 1.5 FDR 0.01.csv")


# Plot dispersions
png("Group1 Vs Group3 qc-dispersions.png", 1000, 1000, pointsize=20)
...
library(gplots)
png("Group1 Vs Group3 qc-heatmap.png", w=1000, h=1000, pointsize=20)
...
png("Group1 Vs Group3 diffexpr-maplot.png", 1500, 1000, pointsize=20)
...
png("Group1 Vs Group3 diffexpr-volcanoplot.png", 1200, 1000, pointsize=20)
volcanoplot(resdata, lfcthresh=1, sigthresh=0.05, textcx=.8, xlim=c(-5, 5), labelsig = FALSE)
dev.off()

##############################################
rm(file, Sample_list)
save.image("Group1_vs_Group3.RData")
