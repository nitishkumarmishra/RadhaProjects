...
annotation <- sapply(tmp, "[[", 1)
meth1 ### After impute.knn of meth
# Change the colname of meth
# a <- strsplit(colnames(meth1), split = "_")
# tmp <- sapply(a, `[`,3)
# ccolnames(meth1) <- tmp
#################################################
...
ht_list = Heatmap(meth1, name = "Methylation", clustering_method_columns = "ward.D2", col = colorRamp2(c(0, 0.10, 0.2, 0.25, 0.50, 0.75, 1), c("darkgreen", "green", "tomato", "violetred", "red2", "red3","red4")),
  ...
  Heatmap(foldchange, name = "Fold Change", col = colorRamp2(c(3.5, 1.0, 0.3), c("red","purple", "blue")), column_names_gp = gpar(fontsize = 8))+
...
pdf("HS-Methylation.pdf", width = 10, height = 10)
