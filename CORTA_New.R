...
CORTA <- read.csv("COARTA Individual 47 targets for Heatmap to Nttish July 3 2016.txt", header = TRUE, sep = "\t", row.names = 1)
rownames(CORTA) <- CORTA$TargetID
BetaCORTA.normal <- CORTA[, grep("Control.AVG_Beta", colnames(CORTA))]
BetaCORTA.affected <- CORTA[, grep("Affected.AVG_Beta", colnames(CORTA))]
Meth <- cbind(BetaCORTA.affected, BetaCORTA.normal)
Meth$TargetID <- rownames(Meth)

FC <- read.csv("1C_COARTA_report 46 targets RBS July 2 2016.txt", header = TRUE, sep = "\t")
...
set.seed(1110)
ha = HeatmapAnnotation(df = data.frame(type = c(rep("CORTA", 23), rep("Control", 16))), col = list(type = c("CORTA" = "red4", "Control"="blue4")))
...
ht_list = Heatmap(Meth1, clustering_method_columns = "ward.D2", name = "Methylation", col = colorRamp2(c(0, 0.1, 0.2, 0.4, 0.5, 0.6, 0.8, 1), c("green4","lightseagreen","aquamarine4", "violet","violetred1","violetred3","firebrick","red4")),
  ...
  Heatmap(logP, name = "log P-value", col = colorRamp2(c(-10, 20, 40), c("red4","gray", "blue4")), column_names_gp = gpar(fontsize = 8))+
...
pdf("CORTA_46.pdf", width = 10, height = 10)
...
save.image(file = "CORTA_New.RData")
